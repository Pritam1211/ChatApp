...
function Avatar() {
  ...
  const setProfilePicture = async ()=> {
    if(selectedAvatar === undefined) {
      ...
      const user = JSON.parse(localStorage.getItem("webchat-user"));
      const {data} = await axios.post(`${host}/${routes.set_avatar}/${user._id}`, {
        image: avatars[selectedAvatar]
      });
      if(data.isSet) {
        user.avatarImage = data.image;
        user.isAvatarImageSet = true;
        localStorage.setItem("webchat-user", JSON.stringify(user));
        navigate("/");
      } else{
